...
app.get('/picFetch', async (req, res) => {
  try{
    let apiResp = await axios.get(`https://www.flicr.com/services/rest/?method=flickr.photos.search&api_key=${api.key}&tags=${req.query.searchTerm}&per_page=15&page=${req.query.pageNumber}`,
        {
          params: {
            format: "json",
            nojsoncallback: "?",
    ...
    imageList = apiResp.data.photos.photo.map( (node) => {
        //photo id
        let id = node.id;
